...
export const TestPage = () =>{ 
  ...
  const viewData = TESTDATA.slice(start, start + viewNum);
  ...
  return( 
    <StyleWrap>
      {viewData.map(item => <p key={item.id}>{item.id}</p>)}
      <Pagination
        page={page}
        totalPages={TESTDATA.length}
        viewCount={viewNum}        // 데이터 5개씩
        center={true}
